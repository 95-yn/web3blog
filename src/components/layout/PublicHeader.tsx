...
export default function PublicHeader() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 ${navBg} backdrop-blur-md border-b ${borderColor}`}>
      <div className="h-16 max-w-4xl mx-auto flex items-center justify-between px-4 md:px-8">
        {/* Logo */}
        <Link href="/" className={`text-lg md:text-xl font-bold ${textColor} ${hoverColor} transition-colors`}>
        ...
        <div className="hidden md:flex items-center gap-6">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className={`text-sm ${textColor} ${hoverColor} transition-colors`}>
              {language === 'zh' ? link.zh : link.en}
            </Link>
          ))}
          <div className="flex items-center gap-2">
            <button onClick={toggleLanguage} className={`px-2 md:px-3 py-1 rounded ${btnBg} transition-colors text-xs md:text-sm`}>
              {language === 'zh' ? 'EN' : '中'}
            </button>
            <button onClick={toggleTheme} className={`px-2 md:px-3 py-1 rounded ${btnBg} transition-colors text-xs md:text-sm`}>
              {isDark ? '☀️' : '🌙'}
            </button>
          </div>
        </div>
        ...
        <div className={`md:hidden ${menuBg} border-t ${borderColor}`}>
          <div className="px-4 py-4 space-y-3">
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={toggleLanguage} className={`flex-1 py-2 rounded ${btnBg} transition-colors text-sm`}>
                {language === 'zh' ? 'EN' : '中'}
              </button>
              <button onClick={toggleTheme} className={`flex-1 py-2 rounded ${btnBg} transition-colors text-sm`}>
                {isDark ? '☀️' : '🌙'}
              </button>
            </div>
          </div>
