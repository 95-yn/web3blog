...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  if (pool.length === 0) {
    return coords.map((c) => ({ ...c, src: "", fullSrc: "", alt: "" }));
  }
...
export default function DomeGallery({
  ...
  grayscale = true,
  autoRotateSpeed = 0,
}: DomeGalleryProps & { autoRotateSpeed?: number }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      if (lastTime !== null && !draggingRef.current && !focusedElRef.current && !inertiaRAF.current) {
