...
export default function PhotoGalleryPage() {
  ...
  const t = {
    zh: { title: '照片墙', back: '返回', done: '恭喜你看完了所有图片，这里就放了这么多' },
    en: { title: 'Photo Gallery', back: 'Back', done: 'Congratulations! You have seen all the photos' },
  }[language]
  ...
  useEffect(() => {
    const gap = 16
    const columns = window.innerWidth > 1100 ? 4 : window.innerWidth > 700 ? 3 : window.innerWidth > 500 ? 2 : 1
  ...
  return (
    <main className={`min-h-screen ${bg} py-20 px-4 md:px-6`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="photo-container relative mt-8" style={{ height: containerHeight }}>
          {positions.map((pos) => (
            <div
              key={pos.num}
        ...
        {selectedImg && (
          ...
          >
            <button className="absolute top-4 right-4 text-white text-4xl hover:text-gray-300 z-10" onClick={(e) => { e.stopPropagation(); setSelectedImg(null) }}>
              ×
            </button>
            <img src={selectedImg} alt="Preview" className="max-w-full max-h-[90vh] object-contain rounded-lg" />
          </div>
        )}
      </div>
    </main>
